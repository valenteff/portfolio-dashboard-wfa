...
class PortfolioBacktestWeight:
    def __init__(self, input_dir, normalized_dir):
        self.input_dir = input_dir
        self.normalized_dir = normalized_dir
        self.wfa_files = {}
        self.equity_files = {}
        self.normalized_data = {}
        self.portfolios_dir = os.path.join(normalized_dir, "portfolios")
        self.top_portfolios = {}
        self.adjusted_monthly_dir = os.path.join(normalized_dir, "adjusted_monthly_analysis")
        self.equity_curves_dir = os.path.join(normalized_dir, "equity_curves")
        self.monthly_dir = os.path.join(normalized_dir, "monthly_analysis")
        self.plots_dir = os.path.join(normalized_dir, "plots")
        self.stats_dir = os.path.join(normalized_dir, "statistics")

        # Create all necessary directories
        for directory in [self.portfolios_dir, self.adjusted_monthly_dir,
                         self.equity_curves_dir, self.monthly_dir, self.plots_dir,
                         self.stats_dir]:
    ...
    def calculate_trade_metrics(self, equity_df):
        ...
        # Recovery factor is the ratio of absolute net profit to absolute max drawdown
        # A higher recovery factor is better (more profit per unit of risk)
        if max_drawdown < 0 and net_profit != 0:
            # Normal case: we have a drawdown and some profit/loss
            recovery_factor = abs(net_profit / max_drawdown)
        elif max_drawdown == 0 and net_profit > 0:
            # Edge case: no drawdown but positive profit (perfect system)
            recovery_factor = 999  # Assign a high value to prioritize these rare cases
        elif max_drawdown == 0 and net_profit < 0:
            # Edge case: no drawdown but negative profit (strange case)
            recovery_factor = 0
        elif max_drawdown == 0 and net_profit == 0:
            # Edge case: no drawdown and no profit (no trades or flat equity)
            recovery_factor = 0
        else:
            # Edge case: positive drawdown (shouldn't happen, but just in case)
            recovery_factor = 0
    ...
    def calculateFinalRank(self, coin_data_list):
        """
        Calculate a weighted final rank for each coin based on multiple performance metrics.

        Args:
            coin_data_list: List of dictionaries containing coin performance data

        Returns:
            List of coins sorted by final rank (lower values are better)
        """
        if not coin_data_list:
            return []

        # Apply filters to exclude coins that don't meet criteria
        filtered_coins = []
        for coin in coin_data_list:
            # Filter 1: Max Drawdown worse than -50%
            if coin['max_drawdown'] < -50:
                print(f"Coin {coin['symbol']} filtered out: Max drawdown {coin['max_drawdown']}% is too high")
                continue

            # Filter 2: Outliers (high win rate but negative profit)
            if coin['win_rate'] > 80 and coin['net_profit'] < 0:  # Relaxed from 70%
                print(f"Coin {coin['symbol']} filtered out: High win rate {coin['win_rate']}% but negative profit {coin['net_profit']}%")
                continue

            filtered_coins.append(coin)

        if not filtered_coins:
            print("Warning: No coins passed the filtering criteria")
            print("Available coins and their metrics:")
            for coin in coin_data_list:
                print(f"Symbol: {coin['symbol']}")
                print(f"  Max Drawdown: {coin['max_drawdown']}%")
                print(f"  Total Trades: {coin['total_trades']}")
                print(f"  Win Rate: {coin['win_rate']}%")
                print(f"  Net Profit: {coin['net_profit']}%")
                print("---")
            return []

        # Calculate individual rankings
        # Sort by each metric and assign ranks (1-based, with ties receiving the same rank)

        # 1. Profit Rank (descending)
        sorted_by_profit = sorted(filtered_coins, key=lambda x: x['net_profit'], reverse=True)
        profit_ranks = {}
        current_rank = 1
        current_value = sorted_by_profit[0]['net_profit'] if sorted_by_profit else 0

        for i, coin in enumerate(sorted_by_profit):
            if coin['net_profit'] != current_value:
                current_rank = i + 1
                current_value = coin['net_profit']
            profit_ranks[coin['symbol']] = current_rank

        # 2. Winning Trades Rank (descending)
        sorted_by_wins = sorted(filtered_coins, key=lambda x: x['winning_trades'], reverse=True)
        wins_ranks = {}
        current_rank = 1
        current_value = sorted_by_wins[0]['winning_trades'] if sorted_by_wins else 0

        for i, coin in enumerate(sorted_by_wins):
            if coin['winning_trades'] != current_value:
                current_rank = i + 1
                current_value = coin['winning_trades']
            wins_ranks[coin['symbol']] = current_rank

        # 3. Recovery Factor Rank (descending)
        sorted_by_recovery = sorted(filtered_coins, key=lambda x: x['recovery_factor'], reverse=True)
        recovery_ranks = {}
        current_rank = 1
        current_value = sorted_by_recovery[0]['recovery_factor'] if sorted_by_recovery else 0

        for i, coin in enumerate(sorted_by_recovery):
            if coin['recovery_factor'] != current_value:
                current_rank = i + 1
                current_value = coin['recovery_factor']
            recovery_ranks[coin['symbol']] = current_rank

        # Calculate weighted final rank for each coin
        ...
        sorted_coins = sorted(filtered_coins, key=lambda x: x['final_rank'])

        return sorted_coins

    def create_top_portfolios(self, num_top_portfolios: int = 10):
        """
        Create top N portfolios based on weighted ranking from the PREVIOUS period.
        """
                    ...
                    for _, row in period_rows.iterrows():
                        ...
                        if symbol in self.equity_files:
                            ...
                            period_symbol_data[period].append({
                                ...
                                'recovery_factor': trade_metrics['recovery_factor'],
        ...
        for n in range(1, num_top_portfolios + 1):
            ...
            for period in all_periods:
                ...
                # Calculate final rank for each coin in the previous period
                ranked_coins = self.calculateFinalRank(prev_period_data)

                # Extract the individual ranking dictionaries from the calculateFinalRank method
                # These will be used to store the individual ranks for each metric
                profit_ranks = {}
                wins_ranks = {}
                recovery_ranks = {}

                # Recalculate the individual ranks to match what was done in calculateFinalRank
                if ranked_coins:
                    # 1. Profit Rank
                    sorted_by_profit = sorted(prev_period_data, key=lambda x: x['net_profit'], reverse=True)
                    current_rank = 1
                    current_value = sorted_by_profit[0]['net_profit']
                    for i, coin in enumerate(sorted_by_profit):
                        if coin['net_profit'] != current_value:
                            current_rank = i + 1
                            current_value = coin['net_profit']
                    ...
                    # 2. Winning Trades Rank
                    sorted_by_wins = sorted(prev_period_data, key=lambda x: x['winning_trades'], reverse=True)
                    current_rank = 1
                    current_value = sorted_by_wins[0]['winning_trades']
                    for i, coin in enumerate(sorted_by_wins):
                        if coin['winning_trades'] != current_value:
                            current_rank = i + 1
                            current_value = coin['winning_trades']
                    ...
                    # 3. Recovery Factor Rank
                    sorted_by_recovery = sorted(prev_period_data, key=lambda x: x['recovery_factor'], reverse=True)
                    current_rank = 1
                    current_value = sorted_by_recovery[0]['recovery_factor']
                    for i, coin in enumerate(sorted_by_recovery):
                        if coin['recovery_factor'] != current_value:
                            current_rank = i + 1
                            current_value = coin['recovery_factor']
                ...
                # Check if we have enough coins for this top-N selection
                if len(ranked_coins) >= n:
                    # Select the nth best coin from the previous period
                    selected_coin = ranked_coins[n-1]  # n-1 because index is 0-based

                    # Get this coin's data for the CURRENT period
                    current_period_data = []
                    for item in period_symbol_data.get(period, []):
                        if item['symbol'] == selected_coin['symbol']:
                            current_period_data.append(item)

                    # If the selected coin has data for the current period, add it to the portfolio
                    if current_period_data:
                        # In case there are multiple entries for the same coin in a period,
                        # choose the one with the best performance based on the same criteria as selection
                        current_period_data.sort(key=lambda x: (x['winning_trades'], x['win_rate'], x['net_profit']), reverse=True)
                        selected_data = current_period_data[0]

                        # Get the individual ranks for this coin from the previous period
                        profit_rank = profit_ranks.get(selected_coin['symbol'], 0)
                        wins_rank = wins_ranks.get(selected_coin['symbol'], 0)
                        recovery_rank = recovery_ranks.get(selected_coin['symbol'], 0)
                        ...
                        portfolio_data.append({
                            ...
                            'max_drawdown': selected_data['max_drawdown'],
                            'recovery_factor': selected_data['recovery_factor'],
                            'selected_based_on_period': prev_period,
                            'rank_in_prev_period': n,
                            'profit_rank_prev_period': profit_rank,
                            'wins_rank_prev_period': wins_rank,
                            'recovery_rank_prev_period': recovery_rank,
                            'final_rank_prev_period': selected_coin['final_rank']
                        })
            ...
            if portfolio_data:
                ...
                avg_recovery = portfolio_df['recovery_factor'].mean()
                avg_max_dd = portfolio_df['max_drawdown'].mean()
                ...
                print(f"  - Recovery factor: {avg_recovery:.2f}")
    ...
    def calculate_monthly_statistics(self, equity_df, portfolio_name):
        ...
        for (year, month), group in equity_df.groupby(['year', 'month']):
            ...
            # We don't need high and low equity for the dashboard compatibility
            # Keeping the code simple to match the wintrades version
    ...
    def generate_equity_curves(self):
        ...
        for portfolio_name, portfolio_df in self.top_portfolios.items():
            ...
            for _, row in portfolio_df.iterrows():
                symbol = row['symbol']
                # Store original period for reference (might be used in future enhancements)
                ...
                # Store end date for reference (might be used for filtering in future enhancements)
        ...
        if all_portfolio_stats:
            ...
            combined_stats_path = os.path.join(self.normalized_dir, "portfolios_comparative_summary.csv")
    ...
    def calculate_trading_statistics(self, equity_df, portfolio_name, portfolio_df):
        ...
        # Recovery Factor
        # Recovery factor is the ratio of absolute total return to absolute max drawdown
        if max_drawdown_pct < 0 and total_return_pct != 0:
            # Normal case: we have a drawdown and some return
            recovery_factor = abs(total_return_pct / max_drawdown_pct)
        elif max_drawdown_pct == 0 and total_return_pct > 0:
            # Edge case: no drawdown but positive return (perfect system)
            recovery_factor = 999  # Assign a high value to prioritize these rare cases
        else:
            # Other edge cases
            recovery_factor = 0
        ...
        if len(daily_returns) > 1:
            ...
            if max_drawdown_pct < 0 and cagr != 0:
                calmar_ratio = abs(cagr / max_drawdown_pct)
            elif max_drawdown_pct == 0 and cagr > 0:
                calmar_ratio = 999  # Assign a high value for perfect systems
            else:
                calmar_ratio = 0
        else:
            volatility = 0
...
if __name__ == "__main__":
    # Example usage
    input_directory = "/Users/fabio/Backtest_Portfolio_WFA_Reformulado/output_20250315_101842"  # Change this to your input directory

    # Create timestamped directory with "weight" prefix
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    normalized_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"results_weight_{timestamp}")

    # Create the normalized directory if it doesn't exist
    if not os.path.exists(normalized_dir):
        os.makedirs(normalized_dir)
        print(f"Created results directory: {normalized_dir}")
